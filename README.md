...
## 🧪 테스트 환경 및 VHAL 시뮬레이터 (Python Injector)

본 프로젝트는 실제 차량 주행 환경을 모사하기 위해 **Python 기반의 VHAL 데이터 시뮬레이터**를 직접 구축하여 테스트를 진행했습니다.


- **VHAL Property ID 우회 맵핑 (RPM 통신 제약 해결):** 테스트 환경의 AAOS 에뮬레이터 내에 Engine RPM을 직접 주입할 수 있는 VHAL 프로퍼티 ID가 등록되지 않은 제약 사항이 있었습니다. 이를 극복하기 위해 프로젝트 내에서 사용하지 않는 `Outside Temperature(외부 온도)` 프로퍼티를 RPM 데이터 전송 통로로 간주(Mapping)하여 시뮬레이터와 안드로이드 앱 간의 데이터 통신을 성공적으로 구현했습니다.

- **테스트 데이터 격리 (Emulator Default Value Filtering):** AAOS 에뮬레이터에는 백그라운드에서 유지되는 자체 기본 VHAL 설정값이 존재합니다. 파이썬 시나리오 스크립트로 주행 데이터를 주입할 때, 이 에뮬레이터의 기본값이 데이터 스트림에 섞여 들어와 연산이 꼬이고 충돌하는 현상이 발생했습니다.

  이를 해결하기 위해 에뮬레이터의 고정된 기본값들을 매직 넘버(Magic Number)로 정의하여 필터링(Ignore)하는 로직을 추가했습니다. (Speed: 9999.0, Fuel: 99999.0, Outside Temperature(RPM): 9999.0)

- **Python Test Script 연동:** `test.py`

  - 터미널을 통해 0.2초 간격으로 시나리오 데이터를 순차적으로 주입하여 앱의 주행 상태 판정 파이프라인을 완벽하게 검증했습니다.
